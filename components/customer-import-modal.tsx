...
export function CustomerImportModal({
  ...
  const handlePreview = async () => {
    ...
    setError('');
    try {
      const data = await previewCustomersImport(file, mode);
      setPreview(data);
    } catch (err) {
  ...
  const handleConfirm = async () => {
    ...
    setError('');
    try {
      const data = await confirmCustomersImport(preview.previewToken);
      setResult(data);
      onSuccess();
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      />
      <div className="relative bg-card rounded-xl border border-border w-full max-w-3xl max-h-[90vh] overflow-hidden flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="text-lg font-bold">{t('importExport.customers.title')}</h2>
        ...
        <div className="p-6 overflow-y-auto space-y-4 flex-1">
          ...
          {result ? (
            <div className="space-y-2 text-sm">
              <p className="font-semibold">{t('importExport.customers.done')}</p>
              <p>{t('importExport.customers.created', { count: result.created })}</p>
              <p>{t('importExport.customers.updated', { count: result.updated })}</p>
              <p>{t('importExport.customers.skipped', { count: result.skipped })}</p>
              {result.failed > 0 && (
                <p className="text-destructive">
                  {t('importExport.customers.failed', { count: result.failed })}
                </p>
              )}
            </div>
          ) : !preview ? (
            <>
              <div>
                <label className="block text-sm font-medium mb-2">{t('importExport.customers.mode')}</label>
                <select
                  value={mode}
                  onChange={(e) => setMode(e.target.value as 'upsert' | 'create_only')}
                  className="w-full px-3 py-2 border border-input rounded-lg bg-background"
                >
                  <option value="upsert">{t('importExport.customers.modeUpsert')}</option>
                  <option value="create_only">{t('importExport.customers.modeCreateOnly')}</option>
                </select>
              </div>
              <label className="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-border rounded-xl p-8 cursor-pointer hover:bg-secondary/50">
                ...
                <span className="text-sm text-muted-foreground">
                  {file ? file.name : t('importExport.customers.selectFile')}
                </span>
                <input
                  type="file"
            ...
            <>
              <div className="grid grid-cols-3 gap-3 text-sm">
                <div className="p-3 bg-secondary rounded-lg">
                  <p className="text-muted-foreground">{t('importExport.customers.total')}</p>
                  <p className="font-bold">{preview.summary.total}</p>
                </div>
                <div className="p-3 bg-green-50 rounded-lg">
                  <p className="text-green-800">{t('importExport.customers.valid')}</p>
                  <p className="font-bold text-green-900">{preview.summary.valid}</p>
                </div>
                <div className="p-3 bg-red-50 rounded-lg">
                  <p className="text-red-800">{t('importExport.customers.errors')}</p>
                ...
                <table className="w-full text-xs">
                  <thead className="bg-secondary sticky top-0">
                    <tr>
                      ...
                      <th className="px-2 py-2 text-left">{t('importExport.customers.colType')}</th>
                      <th className="px-2 py-2 text-left">{t('importExport.customers.colPhone')}</th>
                      <th className="px-2 py-2 text-left">{t('importExport.customers.colName')}</th>
                      <th className="px-2 py-2 text-left">{t('importExport.customers.colTaxCode')}</th>
                  ...
                  <tbody>
                    {preview.rows.map((row) => (
                      <tr key={row.line} className="border-t border-border">
                        ...
                        <td className="px-2 py-1.5">{row.customer_type}</td>
                        <td className="px-2 py-1.5">{row.phone}</td>
                        <td className="px-2 py-1.5">{row.name}</td>
                        <td className="px-2 py-1.5">{row.tax_code ?? ''}</td>
        ...
        <div className="p-6 border-t border-border flex gap-3">
          ...
          {!result && !preview && (
            ...
            >
              ...
              {t('importExport.customers.preview')}
            </button>
          )}
          {!result && preview && (
            <button
            ...
            >
              {isLoading && <Loader2 className="animate-spin" size={16} />}
              {t('importExport.customers.confirm', { count: preview.summary.valid })}
            </button>
          )}
        </div>
      </div>
